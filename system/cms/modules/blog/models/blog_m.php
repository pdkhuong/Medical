...
class Blog_m extends MY_Model
{
  ...
  public function get_group_category() {
      ...
      $this->db
        ->select('blog_categories.title AS category_title, blog_categories.slug AS category_slug, count(1) as total')
        ->join('blog_categories', 'blog.category_id = blog_categories.id', 'right')
    ...
    public function getContinue($id, $limit=1, $isNext=true){
      $whereClause = "blog.id>".$id;
      if($isNext==false){
        $whereClause = "blog.id<".$id;
      }
      $this->db
